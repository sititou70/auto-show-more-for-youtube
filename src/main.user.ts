...
// settings
let scroll_event_interval: number = 200;
const adjust_scroll_px: number = 0;
const target_infos: { name: string; selector: string; click_times: number }[] =
  [
    {
      name: 'comment reply',
      selector:
        'ytd-comment-thread-renderer > div > ytd-comment-replies-renderer > div > ytd-button-renderer',
      click_times: 1,
    },
    {
      name: 'more comment replies',
      selector:
        'ytd-comment-thread-renderer > div > ytd-comment-replies-renderer > div > div > div > yt-next-continuation > tp-yt-paper-button',
      click_times: Infinity,
    },
    {
      name: 'comment read more',
      selector:
        'ytd-comment-thread-renderer ytd-comment-renderer ytd-expander > tp-yt-paper-button',
      click_times: 1,
    },
    {
      name: 'video info',
...
// global variables
let button_caches: {
  [target_name: string]: { elem: Element; click_times: number }[];
} = {};
...
const fireClickEvent = (elem: Element) => {
  if (user_agent.match(/(msie|MSIE)/) || user_agent.match(/(T|t)rident/)) {
    (elem as any).fireEvent('onclick');
  } else {
...
const scrollHandler = () =>
  target_infos.forEach((target_info) => {
    document.querySelectorAll(target_info.selector).forEach((elem) => {
      let button_cahce = button_caches[target_info.name].find(
        (y) => y.elem === elem
      );
      ...
      const click_times_flg =
        button_cahce.click_times < target_info.click_times;
      const button_position_flg =
        elem.getBoundingClientRect().y - window.innerHeight + adjust_scroll_px <
        0;
...
let waiting_for_scroll_event_interval: boolean = false;
...
document.addEventListener('click', (e) => {
  ...
  if (event_target === undefined) return;

  let current_target: HTMLElement | null = event_target;
  while (
    current_target instanceof HTMLElement &&
